...
export default function TambahMapel({ open, onClose, onSubmit, initialData }: any) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      px-8 py-8 animate-scale-in"
      >
        {/* HEADER */}
        <div className="flex items-center justify-between mb-12">
          <h2 className="text-3xl font-bold text-gray-900">{initialData ? "Edit Mapel" : "Tambah Mapel Baru"}</h2>
  ...
  focus:outline-none focus:ring-2 focus:ring-blue-500"
          ...
          <div className="pt-2">
            <button
              disabled={namaMapel.trim() === ""}
              onClick={() => onSubmit({ nama: namaMapel })}
              className={`w-full h-14 rounded-xl font-semibold flex items-center justify-center gap-2 transition
    ${namaMapel.trim() === "" ? "bg-gray-200 text-gray-500 cursor-not-allowed" : "bg-blue-600 hover:bg-blue-700 text-white"}`}
            >
              <Save size={18} />
              Simpan
            </button>
